...
* выделенную память.
* Пример:
*     stringw source(CS(L"test1 test2 test3"));
*     vector<stringw> vec = source.split<vector<stringw>>(CS(L" "));
*     stringw res = +CS(L"res size ") & vec.size() & L":" &
*         e_choice(
*             vec.size() > 0,
*             +CS(L"\n") & e_ls(vec, L", ") & L"\n",
*             +CS(L" ")
...
* Из-коробки такими объектами могут быть sstring, StringConstant, StaticString.
...
* выбор из строковых подвыражений.
*/
template<typename A>
struct strexpr {
